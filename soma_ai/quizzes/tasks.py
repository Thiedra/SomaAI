...
def generate_quiz_task(self, quiz_id: str, question_count: int = 5):
    """
    Celery task: generate quiz questions using Claude AI.
    Saves Question objects to the database on success.
    ...
    try:
        quiz = Quiz.objects.select_related("note__simplified", "student").get(id=quiz_id)

        # get simplified text — quizzes are generated from the simplified version
        try:
            simplified_text = quiz.note.simplified.simplified_text
        except Exception:
            raise ValueError(f"Note {quiz.note.id} has not been simplified yet.")
...
"""
        ...
        # save each question returned by Claude
